import { X, Volume2, VolumeX, Zap, Bell, BellOff, Vibrate, Monitor, Play, Square } from 'lucide-react';
...
export default function SettingsModal({ onClose }) {
  ...
  const tickingSounds = [
    { value: 'none', label: 'None', file: null },
    { value: 'metronome', label: 'Metronome', file: '/Ticking Sounds/Metronome.mp3' },
    { value: 'soft', label: 'Soft Tick', file: '/Ticking Sounds/soft-tick.mp3' },
    { value: 'tick', label: 'Basic Tick', file: '/Ticking Sounds/tick.mp3' },
  ];
  ...
  // Simple sound management functions
  const stopAllSounds = () => {
    // Stop alarm
  ...
  const playTestSound = async (soundType) => {
    try {
      if (soundType === 'alarm') {
        ...
        const soundFile = alarmSounds.find(s => s.value === soundSettings.alarmSound)?.file;
        if (soundFile) {
          console.log('Playing alarm sound:', soundFile);
          alarmAudioRef.current = new Audio(soundFile);
          alarmAudioRef.current.volume = soundSettings.alarmVolume / 100;
          ...
          alarmAudioRef.current.addEventListener('error', (e) => {
            ...
            setIsAlarmPlaying(false);
            alert(`Could not play alarm sound. File may be missing: ${soundFile}`);
          });
        ...
        const soundFile = tickingSounds.find(s => s.value === soundSettings.tickingSound)?.file;
        if (soundFile) {
          console.log('Playing ticking sound:', soundFile);
          tickingAudioRef.current = new Audio(soundFile);
          tickingAudioRef.current.volume = soundSettings.tickingVolume / 100;
          tickingAudioRef.current.loop = true;
          
          tickingAudioRef.current.addEventListener('error', (e) => {
            console.error('Ticking audio error:', e);
            setIsTickingPlaying(false);
            alert(`Could not play ticking sound. File may be missing: ${soundFile}`);
          });
      ...
      const soundName = soundType === 'alarm' 
        ? alarmSounds.find(s => s.value === soundSettings.alarmSound)?.label
        : tickingSounds.find(s => s.value === soundSettings.tickingSound)?.label;
      
      alert(`Could not play ${soundName} sound. Please check file path.`);
    }
  };

  const handleSave = () => {
    // Stop all sounds before saving
    stopAllSounds();
      ...
      <div className="bg-white rounded-3xl p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-2">
            ...
            {(isAlarmPlaying || isTickingPlaying) && (
              <button 
        ...
        <div className="space-y-8">
          ...
          <div>
            ...
            {/* Master sound toggle */}
            <div className="flex justify-between items-center p-3 bg-orange-50 rounded-xl mb-6">
                  ...
                  <div className="flex items-center space-x-2">
                    {isAlarmPlaying && (
                      <button
                        ...
                        className="test-sound-btn p-3 bg-red-100 hover:bg-red-200 rounded-lg transition-all duration-200"
                        title="Stop Sound"
                      >
                    ...
                    <button
                      ...
                      disabled={isAlarmPlaying || soundSettings.alarmSound === 'none'}
                      className={`test-sound-btn p-3 rounded-lg transition-all duration-200 ${
                        isAlarmPlaying || soundSettings.alarmSound === 'none'
                          ? 'bg-gray-100 cursor-not-allowed' 
                          : 'bg-green-100 hover:bg-green-200'
                      }`}
                      title="Test Sound"
                    >
                      <Play size={18} className={isAlarmPlaying || soundSettings.alarmSound === 'none' ? "text-gray-400" : "text-green-600"} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      ...
                      disabled={isTickingPlaying || soundSettings.tickingSound === 'none'}
                      className={`test-sound-btn p-3 rounded-lg transition-all duration-200 ${
                        isTickingPlaying || soundSettings.tickingSound === 'none'
                          ? 'bg-gray-100 cursor-not-allowed' 
                          : 'bg-purple-100 hover:bg-purple-200'
                      }`}
                      title="Test Sound"
                    >
                      <Play size={18} className={isTickingPlaying || soundSettings.tickingSound === 'none' ? "text-gray-400" : "text-purple-600"} />
                    </button>
                  </div>
                </div>
          ...
          <div>
            ...
            <div className="flex justify-between items-center p-3 bg-yellow-50 rounded-xl">
              <label className="text-gray-700 flex items-center space-x-2 font-medium">
                <Zap size={18} className={motivationEnabled ? "text-yellow-500" : "text-gray-400"} />
                <span>Motivational Messages</span>
              </label>
              <button
                onClick={toggleMotivation}
                className={`w-12 h-6 rounded-full transition-colors ${
                  motivationEnabled ? 'bg-yellow-500' : 'bg-gray-300'
                }`}
              >
                <div className={`w-5 h-5 bg-white rounded-full transition-transform ${
